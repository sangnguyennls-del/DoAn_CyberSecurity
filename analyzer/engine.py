...
Hai quyết định thiết kế:
  ...
  2. Cache theo fingerprint: chỉ gửi lên API những finding chưa từng được phân tích.
     Quét lại target cũ gần như miễn phí -> vòng lặp vá->quét lại chạy được nhiều lần.
"""
...
def analyze(
    ...
    """Trả về dict {analyses, summary, priority, warnings}. Không bao giờ raise.

    Lỗi mạng, hết quota, hay bộ lọc từ chối đều biến thành warning: báo cáo vẫn ra,
    chỉ là thiếu cột phân tích. Không đáng để mất cả lần quét đã tốn 10 phút.
    """
    ...
    if conn is not None:
        cached = db_mod.get_cached(conn, [f.fingerprint for f in findings])
